function [fPred,G1hPred,G2hPred,G3hPred,SPred,RedTime] = Principle3Euler(Traindata,N_Tilde,Z_Tilde,Testdata,N,Z,Functionals,FlowType,StepET_R,cnu,Indexstep,FixFunc,CoFunc,InterSc,PlotOpt,Rolie,Metric)
%PRINCIPLE3EULER Uses three chosen functionals alongside the principle
...
%            start time for the Euler method. These columns are ran in 
%            sequence. Note that dt value is measured with respect to GLaMM
%            data, so the default of [1;0] will start the Euler method at
%            time 0 and use a dt value equal to the output from the GLaMM
%            model.
%FixFunc - Vector of 4 logical values to fix gamma_1, gamma_2, gamma_3 or S
...
for k=1:size(Indexstep,2)
    ...
    for i=2:size(StepET_R{3})
        ...
        %Check if timestep to be corrected occurs during Euler method.
    ...
    dG1hPred{k}=zeros(1,RedTlen-1);
    dG2hPred{k}=zeros(1,RedTlen-1);
    dG3hPred{k}=zeros(1,RedTlen-1);
    dSPred{k}=zeros(2,2,RedTlen-1);
    ...
    %Iteratively caluclate G1, G2 and G3 predictions using euler method 
    %with orientation.
    if GFinIndex<RedTlen
        ...
        %5) Find the derivative of S using its formula on f and df.
        dSPred{k}(:,:,i)=dSFunc(fTemp,dfTemp);
        %6) Find the derivative of V using its formula on S and dS.
        dVTemp=MultiVDerivs(SPred{k}(:,:,i),dSPred{k}(:,:,i));
        %7) Find the derivative of fhat using its formula on f,df,S and dS.
        dfhatTemp=MultifhatDerivs(fTemp,dfTemp,VTemp,dVTemp);
        %8) Find the derivatives of the functionals using fhat and dfhat.
        dG1hPred{k}(i)=dG1Func(fhatTemp,dfhatTemp);
        dG2hPred{k}(i)=dG2Func(fhatTemp,dfhatTemp);
        dG3hPred{k}(i)=dG3Func(fhatTemp,dfhatTemp);
        ...
        %9) Perform Euler method for all functionals. (At T=t -> T=t+dt)
        ...
        for i=GFinIndex+1:(RedTlen-1)
            ...
            [fhatTemp,~,NoPts{k}(i)]=Functional3Interpolator(fTVec,G1TVec,...
                G2TVec,G3TVec,G1hPred{k}(i),G2hPred{k}(i),G3hPred{k}(i),N,...
                (i<=PlotI),CoFunc,InterSc);
            %2) Find eigenmatrix V.
            VTemp=Seigen(SPred{k}(:,:,i));
            %3) Apply orientation to calculate f from fhat.
            fTemp=frotate(fhatTemp,VTemp);
            %4) Find the derivative of f using the GLaMM model on f.
            dfTemp=Multifderivs(fTemp,RedTime(i),N,Z,FlowType,StepET_R,cnu,pqmatrix(N));
            %5) Find the derivative of S using its formula on f and df.
            dSPred{k}(:,:,i)=dSFunc(fTemp,dfTemp);
...
%             fhatPred{k}{2,2}(:,:,i)=fhatTemp{2,2};
    ...
    %Plot Euler using predicted Gamma.
    subplot(1+PlotdG,3,1)
    hold on
    if Plotf
...
% [TS9,fS9]=FijpqExtract(GLaMMD('fpq','shear',1,9,75,25,10000,0.1),75,25,'stretchratio','NaN');
% %fhS9=fPrinciple(fS9);
% Principle3Euler([Traindata,Shearhatdata(or(ETRS<=8,ETRS>=10))],75,25,{{TS9,fS9}},75,25,{'NSD','NSDCs','PrinSinSum'},'xySH',{1,9,0},0.1,[1;10],[],'Rad','Rel',[1,1,0,0,0]);